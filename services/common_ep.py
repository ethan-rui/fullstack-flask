...
def page_catalog():
    ...
    if param_search != "":
        products_by_name = db_products.query({"name": param_search})
        products_by_brand = db_products.query({"cat": param_search})
        products_by_category = db_products.query({"brand": param_search})
        products = list(
